  ...
  (cond [(var? e) 
        ...
        [(fst? e)
         (let ([pr (eval-under-env (fst-e e) env)])
           (if (apair? pr) (apair-e1 pr) (error "MUPL fst applied to non-apair")))]
        [(snd? e)
         (let ([pr (eval-under-env (snd-e e) env)])
           (if (apair? pr) (apair-e2 pr) (error "MUPL fst applied to non-apair")))]
        [(isaunit? e)
         ...
         (let ([clo (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clo)
               (letrec ([clo-fun (closure-fun clo)]
                        ...
                        [fun-name (fun-nameopt clo-fun)]
                        [fun-arg (fun-formal clo-fun)]
                        [fun-e (fun-body clo-fun)]
                        [new-env (cons (cons fun-arg arg) clo-env)])
                 (if fun-name
                     (let ([new-new-env (cons (cons fun-name clo) new-env)])
                       (eval-under-env fun-e new-new-env))
                     (eval-under-env fun-e new-env)))
...
(define (mlet* lstlst e2)
  (cond [(null? lstlst) e2]
        [#t (let ([fst-pr (car lstlst)])
              (mlet (car fst-pr) (cdr fst-pr) (mlet* (cdr lstlst) e2)))]))
