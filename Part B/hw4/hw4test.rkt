...
(define tests
  (test-suite
   ...
   (check-equal? (sequence 1 5 1) (list 1 2 3 4 5) "Sequence test")
   ...
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 7) 2 "list-nth-mod test")
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 8) 3 "list-nth-mod test")
   ...
   (check-equal? (stream-for-n-steps ones 0) '() "stream-for-n-steps test")
   ...
   (check-equal? (stream-for-n-steps funny-number-stream 8) (list 1 2 3 4 -5 6 7 8) "funny-number-stream test")
   (check-equal? (stream-for-n-steps funny-number-stream 0) '() "funny-number-stream test")
   ...
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 3) (list (cons 1 "a") (cons 2 "b") (cons 3 "a")) 
                 "cycle-lists test")
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 2) (list (cons 1 "a") (cons 2 "b")) 
                "cycle-lists test")
   ...
   ;(letrec ([fff (cached-assoc (list (cons 1 2) (cons 3 4)) 3)])
   ;  (begin (fff 3) (fff 3)))
